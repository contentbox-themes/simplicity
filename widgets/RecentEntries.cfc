	...
	*/
	any function renderIt(
		...
		saveContent variable="rString"{
			...
			// UL start
			writeOutput('<ul class="recentEntries">');
			// iterate and create
			for(var x=1; x lte arguments.max; x++){
				writeOutput('<li class="recentEntry"><a href="#cb.linkEntry(entryResults.entries[ x ])#">#entryResults.entries[ x ].getTitle()#</a>');
				writeOutput('<span class="entryExcerpt">#entryResults.entries[ x ].getExcerpt()#</span>');
				writeOutput('<span class="entryDate">#entryResults.entries[ x ].getDisplayPublishedDate()#</span>');
				writeOutput('</li>');
			}
			// close ul
			writeOutput( "</ul>" );
